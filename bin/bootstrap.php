 ...
 * (`vendor/tanat/php-mini-http-server/`), that same relative path points at
 * THIS package's own `vendor/` - which a dependency never has one of - and
 * the script fatals before it does anything.
 *
 ...
 * behavior. Run from inside `vendor/tanat/php-mini-http-server/bin/`, the
 * first few levels (this package's own empty `vendor/`, then
 * `vendor/tanat/`, then `vendor/`) all miss, and the walk lands on the
 * CONSUMING project's autoloader - which is what actually has this
 * package's classes loaded into it, via Composer's own merged autoload map.
 */
$directory = __DIR__;
